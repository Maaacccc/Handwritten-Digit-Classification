...
def MLPclassificationLoss(w, X, y, nHidden, nLabels, keep_prob):
    ...
    for i in range(nInstances):
        ...
        for h, hw in enumerate(hiddenWeights):
            ...
            tp = np.tanh(ip[-1])
            # add dropout
            fp[-1] *= np.random.binomial(1, keep_prob, size=fp[-1].shape) #X:[0.5,1,2,3,4,5,6] * r:[0,1,0,1,0,1,0] = []
            fp[-1] /= keep_prob
            fp.append(np.append(tp, 1))
...
for iter in range(maxIter):
    ...
    f, g = MLPclassificationLoss(w, X[i:i+1, :], yExpanded[i:i+1, :], nHidden, nLabels, keep_prob)
