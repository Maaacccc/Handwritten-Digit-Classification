...
def MLPclassificationLoss(w, X, y, nHidden, nLabels):
    ...
    for i in range(nInstances):
        ...
        az = fp[-1] @ outputWeights       
        exp_az = np.exp(az)
        yhat = exp_az / np.sum(exp_az)  
        # y[i] = [0,1,0,0,,,,,,,,]
        # yhat = [0.1,0.1,0.1,0.1]
        ...
        if len(nHidden) > 1:
            ...
            backprop = (np.tile((sech(ip[-1]) ** 2), (nLabels, 1))) * outputWeights.T * err[:, np.newaxis]
            ...
            # gInput += np.outer(X[i], (err @ (np.tile((sech(ip[-1]) ** 2), (nLabels, 1)) * outputWeights.T)))
            gtanh = (sech(ip[-1]) ** 2)
            gtanh = np.tile(gtanh, (nLabels, 1))
            a = gtanh * outputWeights.T
            b = err @ a
